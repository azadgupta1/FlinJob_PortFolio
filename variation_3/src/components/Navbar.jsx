...
const Navbar = () => {
  ...
  return (
<nav className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-white/80 backdrop-blur-lg shadow-md rounded-full px-4 py-2 flex items-center justify-between max-w-4xl w-full z-50 md:top-4 md:px-8 md:py-3">
{/* Logo */}
      <a href="#" className="font-bold text-lg text-gray-900 tracking-wide">MyPortfolio</a>
      ...
      {isOpen && (
        <motion.div
          ...
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.3 }}
          className="absolute top-12 left-0 w-full bg-white shadow-md rounded-xl py-4 flex flex-col items-center gap-4 md:hidden"
        >
          {["about", "experience", "projects", "contact"].map((section) => (
            <a
              key={section}
