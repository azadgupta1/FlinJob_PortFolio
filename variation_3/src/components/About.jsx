    ...
    <section id="about" className="py-16 px-6 bg-gray-200">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <motion.p 
          ...
          whileInView={{ opacity: 1, y: 0 }} 
          transition={{ duration: 0.6 }}
          className="text-gray-700 max-w-3xl mx-auto leading-relaxed text-sm md:text-base"
        >
          I'm a passionate developer who thrives on building innovative and efficient web solutions.
          I specialize in full-stack development, working with <strong>React, Node.js, PostgreSQL, and Tailwind CSS.</strong>
        </motion.p>
        ...
        <div className="mt-12 flex flex-col md:flex-row items-center md:items-start gap-8">
          ...
          {/* Skills Grid */}
          <motion.div 
            className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full md:w-auto"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7 }}
          >
            {[
              ...
              <motion.div 
                key={index} 
                whileHover={{ scale: 1.05 }} 
                className="bg-gray-100 p-4 md:p-6 rounded-xl shadow-md text-left"
              >
                <h3 className="text-base md:text-lg font-semibold text-gray-900">{item.title}</h3>
                <p className="text-sm md:text-base text-gray-700 mt-2">{item.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
